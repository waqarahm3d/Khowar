...
import { getSong, getArtists, getAlbums, updateSong, uploadAudio, uploadImage } from '../services/api'
...
export default function EditSong() {
  ...
  const updateMutation = useMutation({
    mutationFn: (data) => updateSong(id, data),
    onSuccess: () => {
  ...
  if (isLoading) {
    return (
      <div className="p-8">
        <div className="text-center py-12">Loading song...</div>
      </div>
    )
  }
    ...
    <div className="p-8">
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl">
        <div className="bg-white rounded-lg shadow p-6 space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Audio File (leave empty to keep current)
            </label>
            <input
              type="file"
              accept="audio/*"
          ...
          >
            {uploading ? 'Uploading...' : updateMutation.isPending ? 'Updating...' : 'Update Song'}
          </button>
        </div>
      </form>
    </div>
