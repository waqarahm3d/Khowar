        ...
        <form onSubmit={handleSearch} className="max-w-2xl">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-14 pr-4 py-4 bg-white/10 border border-white/20 rounded-full text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500"
            />
          </div>
        </form>
      </div>
                ...
                <div className="space-y-12">
                  ...
                  {(songs[0] || artists[0] || albums[0]) && (
                    <section>
                      ...
                      <div className="bg-primary-800/30 rounded-lg p-6 hover:bg-primary-800/40 transition-colors cursor-pointer">
                        {songs[0] && (
                          <div onClick={() => handlePlaySong(songs[0], songs)}>
                            ...
                            />
                            <h3 className="text-3xl font-bold text-white mb-2">{songs[0].title}</h3>
                            <p className="text-gray-300">Song • {songs[0].artist?.name}</p>
                          </div>
                        )}
                        {!songs[0] && artists[0] && (
                          <a href={`/artist/${artists[0]._id}`}>
                            ...
                            />
                            <h3 className="text-3xl font-bold text-white mb-2">{artists[0].name}</h3>
                        ...
                        {!songs[0] && !artists[0] && albums[0] && (
                          <a href={`/album/${albums[0]._id}`}>
                            <img
                              ...
                              className="w-24 h-24 rounded-lg mb-4"
                            />
                            <h3 className="text-3xl font-bold text-white mb-2">{albums[0].title}</h3>
                            <p className="text-gray-300">Album • {albums[0].artist?.name}</p>
                          </a>
                        )}
                      </div>
                    </section>
              ...
              {activeTab === 'songs' && (
                <section>
                  {songs.length > 0 ? (
                    ...
                    <p className="text-gray-400 text-center py-12">No songs found</p>
                  )}
                </section>
              )}
              ...
              {activeTab === 'albums' && (
                <section>
                  {albums.length > 0 ? (
                    ...
                    <p className="text-gray-400 text-center py-12">No albums found</p>
                  )}
                </section>
              )}
              ...
              {activeTab === 'playlists' && (
                <section>
                  {playlists.length > 0 ? (
                    ...
                    <p className="text-gray-400 text-center py-12">No playlists found</p>
                  )}
                </section>
              )}
            </>
